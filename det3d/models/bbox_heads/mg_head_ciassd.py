...
class MultiGroupHead(nn.Module):
    def __init__(self,
        ...
        from det3d.models.losses.gious import gious_3D
        self.gious_3d_loss = gious_3D()
    ...
    def loss(self, example, preds_dicts, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):  # task_id: 0
            ...
            # for iou3d loss
            # pos_pred_mask = reg_weights > 0
            # gious_loss = torch.tensor(0.0, dtype=torch.float32, device=loss.device)
            # if pos_pred_mask.sum() > 0:
            #     qboxes = self.box_coder.decode_torch(preds_dict["box_preds"].view(batch_size, -1, 7)[pos_pred_mask], example["anchors"][0][pos_pred_mask])
            #
            #     # todo: sometimes the prediction create nan output, I don't know the exact reason and just doubt it may be caused by inproper gradient in the training
            #     nan_mask = torch.isnan(qboxes).any(-1)
            #     non_nan_mask = torch.logical_not(nan_mask)
            #     nan_sum = nan_mask.sum()
            #     if nan_sum > 0:
            #         print("nan_sum: ", nan_sum)
            #
            #     if non_nan_mask.sum() == 0:
            #         gious_loss = torch.tensor(0.0, dtype=torch.float32, device=loss.device)
            #     else:
            #         qboxes = qboxes[non_nan_mask]
            #         gboxes = self.box_coder.decode_torch(example["reg_targets"][0][pos_pred_mask], example["anchors"][0][pos_pred_mask])[non_nan_mask]
            #         weights = reg_weights[pos_pred_mask][non_nan_mask]
            #         batch_gious = self.gious_3d_loss(gboxes, qboxes, False).squeeze() * weights  # averaged in sample
            #         gious_loss = 1 - batch_gious.sum() / batch_size                              # averaged in batch
            # loss += gious_loss * 2.0
            ...
            ret = {
                ...
                # "ious_loss": gious_loss.detach().cpu(),
    ...
    def get_task_detections(self, task_id, num_class_with_bg, test_cfg, batch_cls_preds, batch_reg_preds,
        ...
        for box_preds, cls_preds, dir_preds, iou_preds, a_mask, meta, valid_frustum in zip(batch_reg_preds,  batch_cls_preds, batch_dir_preds,
            ...
            if test_cfg.score_threshold > 0.0:
                ...
                # todo: replace classification score with iou prediction.
                iou_preds = (iou_preds.squeeze() + 1) * 0.5
                top_scores *= torch.pow(iou_preds.masked_select(top_scores_keep), 4)
                # extra centerness from dinms
                # dist_boxes_anchors = torch.abs(box_preds[top_scores_keep] - anchors[top_scores_keep])
                ...
                # top_scores *= torch.pow(dist_metric, 2)
            ...
            if top_scores.shape[0] != 0:
                ...
                elif nms_type == 'rotate_weighted_nms':
