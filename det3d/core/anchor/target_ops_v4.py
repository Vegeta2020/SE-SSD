...
def create_target_np(
    ...
    if len(gt_boxes) > 0:
        ...
        anchor_to_gt_max = anchor_by_gt_overlap[np.arange(num_inside), anchor_to_gt_argmax]  # (N,), get the largest iou with one gt_box for each anchor
    ...
    labels_full = - np.ones((total_anchors,), dtype=np.int32)
    bbox_targets_full = np.zeros((total_anchors, box_code_size), dtype=all_anchors.dtype)
    bbox_outside_weights_full = np.zeros((total_anchors,), dtype=all_anchors.dtype)

    labels_full[inds_inside] = labels
    bbox_targets_full[inds_inside, :] = bbox_targets
    bbox_outside_weights_full[inds_inside] = bbox_outside_weights

    ret = { "labels": labels_full,                             # [70400,]
            "bbox_targets": bbox_targets_full,                 # [num_pos_anchors, 7]
            "bbox_outside_weights": bbox_outside_weights_full, # [70400,]
