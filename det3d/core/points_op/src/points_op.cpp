...
int pts_in_boxes3d_cpu_v2(at::Tensor pts, at::Tensor boxes3d, at::Tensor pts_flag, at::Tensor reg_target){
    // param pts: (N, 3)
    // param boxes3d: (M, 7)  [x, y, z, h, w, l, ry]
    // param pts_flag: (M, N)
    // param reg_target: (N, 3), center offsets
    ...
    for (i = 0; i < boxes_num; i++){
        for (j = 0; j < pts_num; j++){
            ...
            if(cur_in_flag==1){
                reg_target_flat[j*3] = pts_flat[j*3] - boxes3d_flat[i*7];
                reg_target_flat[j*3+1] = pts_flat[j*3+1] - boxes3d_flat[i*7+1];
                reg_target_flat[j*3+2] = pts_flat[j*3+2] - (boxes3d_flat[i*7+2] + boxes3d_flat[i*7+5] / 2.0);
            }
        }
    }
    return 1;
...
int pts_in_boxes3d_cpu_v3(at::Tensor pts, at::Tensor boxes3d, at::Tensor pts_flag, at::Tensor reg_target){
    // param pts: (N, 3)
    // param boxes3d: (M, 7)  [x, y, z, h, w, l, ry]
    // param pts_flag: (M, N)
    // param reg_target: (N, 3), center offsets
...
//    memset(reg_target_flat, 0, pts_num * sizeof(float));
    ...
    float px, py, pz;
    for (i = 0; i < boxes_num; i++){
        float angle = boxes3d_flat[i * 7 + 6];
        ...
        for (j = 0; j < pts_num; j++){
            ...
            pts_flag_flat[i * pts_num + j] = cur_in_flag;
            if(cur_in_flag==1){
                px = pts_flat[j*3] - boxes3d_flat[i*7];
                py = pts_flat[j*3+1] - boxes3d_flat[i*7+1];
                pz = pts_flat[j*3+2] - (boxes3d_flat[i*7+2] + boxes3d_flat[i*7+5] / 2.0);
                reg_target_flat[j*3] = ((px * cosa + py * (-sina)) / boxes3d_flat[i*7+3]) + 0.5;
                reg_target_flat[j*3+1] = ((px * sina + py * cosa) / boxes3d_flat[i*7+4]) + 0.5;
                reg_target_flat[j*3+2] = (pz / boxes3d_flat[i*7+5]) + 0.5;
            }
        }
    }
    return 1;
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("pts_in_boxes3d_v2", &pts_in_boxes3d_cpu_v2, "points in boxes3d (CPU)");
  m.def("pts_in_boxes3d_v3", &pts_in_boxes3d_cpu_v3, "points in boxes3d (CPU)");
}