...
def noise_per_object_v5_(gt_boxes, points=None, valid_mask=None, rotation_perturb=np.pi / 4, center_noise_std=1.0, \
                         global_random_rot_range=np.pi / 4, num_try=5, group_ids=None, data_aug_with_context=-1.0, \
                         data_aug_random_drop=-1.0):
    """
        perform random rotation and translation on each groundtruth independently.
    ...
    # gt_boxes: x,y,z(lidar), w, l, h, ry(cam).
    gt_box_corners = box_np_ops.center_to_corner_box3d(gt_boxes[:, :3], gt_boxes[:, 3:6] + offset[2:5], gt_boxes[:, 6],
    ...
    return loc_transforms, rot_transforms, valid_mask
...
def random_flip_v2(gt_boxes, points, probability=0.5):
    enable = np.random.choice([False, True], replace=False, p=[1 - probability, probability])
    if enable:
        if gt_boxes is not None:
            ...
            gt_boxes[:, -1] = -gt_boxes[:, -1] + np.pi
        points[:, 1] = -points[:, 1]
        if gt_boxes is not None and gt_boxes.shape[1] > 7:  # y axis: x, y, z, w, h, l, vx, vy, r
            gt_boxes[:, 7] = -gt_boxes[:, 7]
...
def global_scaling_v3(gt_boxes, points, min_scale=0.95, max_scale=1.05):
    noise_scale = np.random.uniform(min_scale, max_scale)
    points[:, :3] *= noise_scale
    if gt_boxes is not None:
        gt_boxes[:, :-1] *= noise_scale
...
def global_rotation_v3(gt_boxes, points, rotation=np.pi / 4):
    if not isinstance(rotation, list):
        rotation = [-rotation, rotation]
    noise_rotation = np.random.uniform(rotation[0], rotation[1])
    points[:, :3] = box_np_ops.rotation_points_single_angle(points[:, :3], noise_rotation, axis=2)
...
def global_translate_v2(gt_boxes, points, noise_translate_std):
    """
    Apply global translation to gt_boxes and points.
    """
    ...
    noise_translate = np.array(
        [
            np.random.normal(scale=noise_translate_std[0], size=1),
            np.random.normal(scale=noise_translate_std[1], size=1),
            np.random.normal(scale=noise_translate_std[2], size=1),
        ]
    ).T
